const mySkills = [
     {
          title: 'NEXT.JS',
          text: 'ssaas',
          icon: (
               <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="20"
                    height="20"
                    preserveAspectRatio="xMidYMid"
                    viewBox="0 0 256 256"
               >
                    <path d="M119.617.069c-.55.05-2.302.225-3.879.35-36.36 3.278-70.419 22.894-91.99 53.044-12.012 16.764-19.694 35.78-22.597 55.922C.125 116.415 0 118.492 0 128.025s.125 11.61 1.151 18.64c6.957 48.065 41.165 88.449 87.56 103.411 8.309 2.678 17.067 4.504 27.027 5.605 3.879.425 20.645.425 24.524 0 17.192-1.902 31.756-6.155 46.12-13.486 2.202-1.126 2.628-1.426 2.327-1.677-.2-.15-9.584-12.735-20.845-27.948l-20.47-27.648-25.65-37.956c-14.114-20.868-25.725-37.932-25.825-37.932-.1-.025-.2 16.84-.25 37.431-.076 36.055-.1 37.506-.551 38.357-.65 1.226-1.151 1.727-2.202 2.277-.801.4-1.502.475-5.28.475h-4.33l-1.15-.725a4.7 4.7 0 0 1-1.677-1.827l-.526-1.126.05-50.166.075-50.192.776-.976c.4-.525 1.251-1.2 1.852-1.526 1.026-.5 1.426-.55 5.755-.55 5.105 0 5.956.2 7.282 1.651.376.4 14.264 21.318 30.88 46.514 16.617 25.195 39.34 59.599 50.5 76.488l20.27 30.7 1.026-.675c9.084-5.905 18.693-14.312 26.3-23.07 16.191-18.59 26.626-41.258 30.13-65.428 1.026-7.031 1.151-9.108 1.151-18.64 0-9.534-.125-11.61-1.151-18.641-6.957-48.065-41.165-88.449-87.56-103.411-8.184-2.652-16.892-4.479-26.652-5.58-2.402-.25-18.943-.525-21.02-.325m52.401 77.414c1.201.6 2.177 1.752 2.527 2.953.2.65.25 14.562.2 45.913l-.074 44.987-7.933-12.16-7.958-12.16v-32.702c0-21.143.1-33.028.25-33.603.4-1.401 1.277-2.502 2.478-3.153 1.026-.525 1.401-.575 5.33-.575 3.704 0 4.354.05 5.18.5"></path>
               </svg>
          ),
     },
     {
          title: 'REACT.JS',
          text: 'fdgdfgsdf',
          icon: (
               <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="20"
                    height="20"
                    fill="none"
                    viewBox="0 0 32 32"
               >
                    <path
                         fill="#53C1DE"
                         d="M18.679 15.976c0-1.435-1.2-2.597-2.679-2.597-1.48 0-2.679 1.162-2.679 2.597 0 1.434 1.2 2.597 2.679 2.597 1.48 0 2.679-1.163 2.679-2.597"
                    ></path>
                    <path
                         fill="#53C1DE"
                         fillRule="evenodd"
                         d="M24.7 11.154c.566-2.23 1.277-6.363-1.23-7.764-2.495-1.395-5.742 1.278-7.456 2.883-1.71-1.589-5.048-4.25-7.552-2.845-2.494 1.4-1.725 5.465-1.147 7.708-2.327.64-6.315 2.02-6.315 4.84 0 2.811 3.984 4.313 6.297 4.953-.58 2.255-1.311 6.262 1.186 7.659 2.514 1.405 5.842-1.194 7.572-2.816 1.726 1.614 4.942 4.23 7.437 2.83 2.504-1.406 1.852-5.484 1.273-7.74 2.242-.641 6.235-2.11 6.235-4.886 0-2.793-4.01-4.184-6.3-4.822m-.284 8.513a29 29 0 0 0-1.519-3.685 29 29 0 0 0 1.46-3.631c1.679.472 5.361 1.55 5.361 3.625 0 2.094-3.533 3.183-5.302 3.691m-1.566 7.859c-1.862 1.045-4.628-1.456-5.902-2.645a29 29 0 0 0 2.514-3.096c1.45-.125 2.821-.329 4.064-.607.407 1.595 1.194 5.298-.676 6.348m-13.726-.015c-1.863-1.041-1.011-4.616-.584-6.278 1.228.264 2.589.453 4.042.568.83 1.132 1.7 2.173 2.576 3.085-1.083 1.015-4.163 3.671-6.034 2.625M2.28 15.976c0-2.102 3.661-3.173 5.378-3.643a29 29 0 0 0 1.462 3.673 30 30 0 0 0-1.48 3.726c-1.637-.454-5.36-1.653-5.36-3.756M9.104 4.504c1.87-1.05 4.77 1.506 6.012 2.656a30 30 0 0 0-2.556 3.066c-1.41.127-2.761.33-4.003.604-.466-1.81-1.321-5.278.547-6.326m11.275 7.073c.957.117 1.875.273 2.736.464-.259.803-.581 1.643-.96 2.504a41 41 0 0 0-1.776-2.968m-4.365-3.529c.591.621 1.183 1.314 1.766 2.066a39 39 0 0 0-3.546 0 28 28 0 0 1 1.78-2.066m-6.157 6.496a27 27 0 0 1-.954-2.517 30 30 0 0 1 2.72-.452 36 36 0 0 0-1.766 2.97m1.793 5.922a28 28 0 0 1-2.764-.431c.264-.83.59-1.692.972-2.568a36 36 0 0 0 1.792 2.999m4.4 3.525a28 28 0 0 1-1.805-2.094q1.783.069 3.566-.006a27 27 0 0 1-1.761 2.1m6.117-6.569c.4.886.739 1.744 1.007 2.559a28 28 0 0 1-2.798.462c.639-.982 1.239-1.99 1.79-3.02m-3.42 3.172a41.5 41.5 0 0 1-5.463.01 35 35 0 0 1-2.746-4.598 35 35 0 0 1 2.73-4.59c1.82-.133 3.65-.134 5.469 0a39 39 0 0 1 2.739 4.572 39 39 0 0 1-2.729 4.606M22.83 4.467c1.87 1.046 1.038 4.76.63 6.376a32 32 0 0 0-4.012-.615 29 29 0 0 0-2.536-3.069c1.258-1.175 4.066-3.728 5.918-2.692"
                         clipRule="evenodd"
                    ></path>
               </svg>
          ),
     },
     {
          title: 'JAVASCRIPT',
          text: 'jfgljfdlg;ertretre',
          icon: (
               <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="20"
                    height="20"
                    preserveAspectRatio="xMinYMin meet"
                    viewBox="0 0 256 256"
               >
                    <path fill="#F7DF1E" d="M0 0h256v256H0z"></path>
                    <path d="m67.312 213.932 19.59-11.856c3.78 6.701 7.218 12.371 15.465 12.371 7.905 0 12.89-3.092 12.89-15.12v-81.798h24.057v82.138c0 24.917-14.606 36.259-35.916 36.259-19.245 0-30.416-9.967-36.087-21.996m85.07-2.576 19.588-11.341c5.157 8.421 11.859 14.607 23.715 14.607 9.969 0 16.325-4.984 16.325-11.858 0-8.248-6.53-11.17-17.528-15.98l-6.013-2.58c-17.357-7.387-28.87-16.667-28.87-36.257 0-18.044 13.747-31.792 35.228-31.792 15.294 0 26.292 5.328 34.196 19.247l-18.732 12.03c-4.125-7.389-8.591-10.31-15.465-10.31-7.046 0-11.514 4.468-11.514 10.31 0 7.217 4.468 10.14 14.778 14.608l6.014 2.577c20.45 8.765 31.963 17.7 31.963 37.804 0 21.654-17.012 33.51-39.867 33.51-22.339 0-36.774-10.654-43.819-24.574"></path>
               </svg>
          ),
     },
     {
          title: 'TAILWIND',
          text: 'rterertytytrytr',
          icon: (
               <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="20"
                    height="20"
                    viewBox="3 2 25 25"
               >
                    <path
                         fill="#1494a2"
                         d="M9 13.7q1.4-5.6 7-5.6c5.6 0 6.3 4.2 9.1 4.9q2.8.7 4.9-2.1-1.4 5.6-7 5.6c-5.6 0-6.3-4.2-9.1-4.9q-2.8-.7-4.9 2.1m-7 8.4q1.4-5.6 7-5.6c5.6 0 6.3 4.2 9.1 4.9q2.8.7 4.9-2.1-1.4 5.6-7 5.6c-5.6 0-6.3-4.2-9.1-4.9q-2.8-.7-4.9 2.1"
                    ></path>
               </svg>
          ),
     },
     {
          title: 'REACT QUERY',
          text: 'kdkakljltjljljlj',
          icon: (
               <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="20"
                    height="20"
                    viewBox="0 0 256 230"
               >
                    <path
                         fill="#00435b"
                         d="m157.98 142.487-4.91 8.527a8.29 8.29 0 0 1-7.182 4.151H108.27a8.29 8.29 0 0 1-7.182-4.151l-4.911-8.527zm13.747-23.87-8.658 15.034h-71.98l-8.658-15.034zm-8.34-23.342 8.354 14.506H82.417l8.354-14.506zm-17.5-22.066a8.29 8.29 0 0 1 7.183 4.151l5.228 9.079H95.86l5.229-9.079a8.29 8.29 0 0 1 7.182-4.151z"
                    ></path>
                    <path
                         fill="#002b3b"
                         d="M53.523 69.252c-4.167-20.206-5.062-35.704-2.368-46.957 1.602-6.693 4.53-12.153 8.984-16.093 4.702-4.159 10.646-6.2 17.326-6.2 11.018 0 22.602 5.025 34.98 14.57 5.05 3.894 10.29 8.587 15.732 14.082.434-.557.923-1.083 1.469-1.57 15.386-13.71 28.34-22.23 39.42-25.514 6.588-1.954 12.773-2.14 18.405-.244 5.946 2 10.683 6.137 14.026 11.93 5.516 9.561 6.97 22.124 4.914 37.637-.838 6.323-2.271 13.21-4.296 20.673.764.092 1.53.262 2.288.513 19.521 6.47 33.345 13.426 41.714 21.377 4.98 4.73 8.231 9.996 9.407 15.826 1.24 6.153.03 12.324-3.308 18.113-5.506 9.548-15.63 17.077-30.052 23.041-5.79 2.395-12.343 4.564-19.664 6.515.334.754.594 1.555.767 2.395 4.167 20.206 5.061 35.704 2.368 46.957-1.602 6.693-4.531 12.153-8.985 16.093-4.701 4.159-10.646 6.2-17.325 6.2-11.019 0-22.602-5.025-34.98-14.57-5.104-3.936-10.402-8.687-15.907-14.258a11.7 11.7 0 0 1-2.084 2.442c-15.386 13.712-28.34 22.23-39.42 25.515-6.588 1.954-12.773 2.14-18.405.244-5.946-2-10.683-6.137-14.026-11.93-5.516-9.561-6.97-22.124-4.914-37.637.869-6.551 2.376-13.709 4.518-21.485a11.7 11.7 0 0 1-2.51-.537c-19.521-6.47-33.345-13.426-41.714-21.377-4.98-4.73-8.231-9.996-9.407-15.826-1.24-6.153-.03-12.325 3.308-18.114 5.506-9.547 15.63-17.077 30.052-23.04 5.963-2.467 12.734-4.693 20.32-6.689a12 12 0 0 1-.633-2.082"
                    ></path>
                    <path
                         fill="#ff4154"
                         d="M189.647 161.333a3.684 3.684 0 0 1 4.235 2.81l.023.112.207 1.075q10.065 52.915-14.18 52.915-23.72 0-60.392-45.153a3.684 3.684 0 0 1 2.777-6.005h.114l1.288.009q15.432.084 30.004-1.076 17.2-1.37 35.924-4.687M78.646 134.667l.062.105.646 1.127q7.765 13.5 16.18 25.627 9.912 14.28 22.29 28.914a3.684 3.684 0 0 1-.309 5.082l-.093.083-.83.715q-40.96 35.096-53.244 14.012-12.025-20.636 8.719-75.047a3.683 3.683 0 0 1 6.579-.618m124.857-52.054.112.037 1.028.354q50.557 17.588 38.416 38.655-11.874 20.605-69.041 30.004a3.683 3.683 0 0 1-3.773-5.501q8.188-13.928 14.749-27.717 7.44-15.638 13.965-33.57a3.684 3.684 0 0 1 4.432-2.295zM84.446 76.71a3.683 3.683 0 0 1 1.31 5.042q-8.19 13.927-14.75 27.717-7.44 15.637-13.965 33.57a3.684 3.684 0 0 1-4.544 2.262l-.112-.037-1.028-.355Q.8 127.322 12.941 106.255 24.815 85.65 81.982 76.25c.85-.14 1.722.022 2.464.459m108.206-57.748q12.025 20.637-8.719 75.048a3.683 3.683 0 0 1-6.579.618l-.062-.105-.646-1.127q-7.765-13.5-16.18-25.627-9.912-14.28-22.29-28.914a3.684 3.684 0 0 1 .309-5.082l.093-.083.83-.715q40.96-35.095 53.244-14.013M77.45 10.59q23.721 0 60.392 45.152a3.684 3.684 0 0 1-2.777 6.005h-.114l-1.288-.008q-15.431-.084-30.003 1.076-17.202 1.37-35.925 4.687a3.684 3.684 0 0 1-4.234-2.81l-.024-.113-.207-1.074Q53.204 10.59 77.45 10.59"
                    ></path>
                    <path
                         fill="#ffd94c"
                         d="M111.295 73.67h31.576a12.89 12.89 0 0 1 11.181 6.475l15.855 27.626a12.89 12.89 0 0 1 0 12.834l-15.855 27.626a12.89 12.89 0 0 1-11.181 6.475h-31.576c-4.618 0-8.883-2.47-11.182-6.475L84.26 120.605a12.89 12.89 0 0 1 0-12.834l15.854-27.626a12.89 12.89 0 0 1 11.182-6.475m26.763 8.338c4.62 0 8.888 2.473 11.185 6.481l11.056 19.288a12.89 12.89 0 0 1 0 12.822l-11.056 19.288a12.89 12.89 0 0 1-11.185 6.48h-21.95c-4.62 0-8.888-2.472-11.185-6.48l-11.056-19.288a12.89 12.89 0 0 1 0-12.822l11.056-19.288a12.89 12.89 0 0 1 11.184-6.48zm-5.187 9.12h-11.576a12.89 12.89 0 0 0-11.179 6.47l-5.842 10.167a12.89 12.89 0 0 0 0 12.846l5.842 10.168a12.89 12.89 0 0 0 11.179 6.47h11.576c4.616 0 8.88-2.468 11.179-6.47l5.842-10.168a12.89 12.89 0 0 0 0-12.846l-5.842-10.168a12.89 12.89 0 0 0-11.179-6.47m-4.994 8.729c4.612 0 8.873 2.464 11.173 6.46l.829 1.44a12.89 12.89 0 0 1 0 12.862l-.829 1.44a12.89 12.89 0 0 1-11.173 6.46h-1.588a12.89 12.89 0 0 1-11.173-6.46l-.829-1.44a12.89 12.89 0 0 1 0-12.862l.829-1.44a12.89 12.89 0 0 1 11.173-6.46zm-.792 8.599a5.74 5.74 0 0 0-4.97 2.866 5.73 5.73 0 0 0 0 5.732 5.738 5.738 0 0 0 9.937 0 5.73 5.73 0 0 0 0-5.732 5.74 5.74 0 0 0-4.967-2.866m-46.509 5.732h10.32"
                    ></path>
               </svg>
          ),
     },
     {
          title: 'GIT',
          text: `An In-Depth Look at Git
Introduction

Git is a distributed version control system designed to track changes in source code during software development. It allows multiple developers to work on a project simultaneously without interfering with each other's work. Due to its speed, efficiency, and ability to handle branching and merging, Git has become one of the most popular version control systems in the world.

History of Git

Git was developed by Linus Torvalds, the creator of Linux, in 2005. Before Git, the Linux kernel project used BitKeeper, a proprietary version control system. Due to a licensing dispute between the community and BitKeeper, Torvalds decided to create a new version control system that would be free and open-source. Within a few days, Torvalds had developed a functional version of Git, and it quickly became the go-to tool for many developers.

Development and Evolution of Git

Torvalds designed Git with speed, simplicity, and robust support for distributed, non-linear workflows in mind. It is designed to handle large projects with speed and efficiency. Since its inception, Git has been improved and extended by a large community of developers. Today, it is maintained by Junio Hamano and many other contributors who ensure its continuous development and stability.

Key Features of Git

Distributed System: Every copy of a Git repository contains the entire history of the project, making it easy to work offline and independently.

Branching and Merging: Git's branching model is straightforward and flexible, allowing developers to create, merge, and delete branches without disrupting the main project.

Efficient Performance: Git handles large codebases and many contributors with ease, thanks to its efficient storage and retrieval mechanisms.

Data Integrity: Git ensures the integrity of source code with checksums, making it almost impossible to lose information.

Contributors and Companies

Linus Torvalds: The initial developer of Git, who created it to manage the Linux kernel development.

Junio Hamano: A significant contributor and the current maintainer of Git, overseeing its development and releases.

Community of Developers: Thousands of developers worldwide contribute to the continuous improvement and extension of Git.

How Git Works

Git tracks changes in the form of snapshots of the entire repository. Each commit in Git represents a point in the project's history, and developers can switch between these points using branches and tags. Git uses SHA-1 hashes to ensure the integrity of data and prevent corruption. The fundamental workflow in Git involves creating branches for new features, making commits to track changes, and merging these branches back into the main project.

Usage of Git

Git is widely used in both open-source and private projects. It is the backbone of popular platforms like GitHub, GitLab, and Bitbucket, which provide additional features such as code review, issue tracking, and continuous integration. Developers use Git for collaborative development, allowing multiple contributors to work on the same project simultaneously without conflicts.

Conclusion

Git has revolutionized the way developers manage and collaborate on code. Its distributed nature, efficient performance, and robust branching model make it an essential tool for modern software development. Whether you're working on a small personal project or a large enterprise application, Git provides the tools and flexibility needed to manage your code effectively.

Further Reading

For those interested in learning more about Git, the following resources are highly recommended:

Pro Git Book by Scott Chacon and Ben Straub

Git Documentation available on the official Git website

Online Tutorials and Courses on platforms like Coursera, Udemy, and Codecademy.`,
          icon: (
               <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="20"
                    height="20"
                    viewBox="0 0 256 256"
               >
                    <g fill="none">
                         <rect
                              width="256"
                              height="256"
                              fill="#f03c2e"
                              rx="60"
                         ></rect>
                         <g clipPath="url(#skillIconsGit0)">
                              <path
                                   fill="#fff"
                                   d="m224.225 119.094-87.319-87.319a12.87 12.87 0 0 0-14.035-2.793 12.9 12.9 0 0 0-4.177 2.793L100.569 49.9l23 23c5.35-1.875 11.475-.594 15.737 3.669a15.31 15.31 0 0 1 3.631 15.831l22.169 22.169c5.363-1.85 11.55-.657 15.831 3.637a15.32 15.32 0 0 1 3.321 16.706 15.333 15.333 0 0 1-20.029 8.293c-1.86-.771-3.55-1.9-4.973-3.324-4.5-4.5-5.612-11.125-3.337-16.669l-20.675-20.675v54.407a15.6 15.6 0 0 1 4.062 2.9 15.326 15.326 0 0 1-21.675 21.675 15.32 15.32 0 0 1-3.326-16.704 15.3 15.3 0 0 1 3.326-4.971c1.481-1.475 3.125-2.594 5.019-3.344v-54.913a15.2 15.2 0 0 1-5.019-3.343 15.315 15.315 0 0 1-3.3-16.757L91.644 58.814l-59.875 59.812a12.88 12.88 0 0 0-2.795 14.04 12.9 12.9 0 0 0 2.795 4.179l87.325 87.312a12.9 12.9 0 0 0 4.177 2.793 12.9 12.9 0 0 0 9.858 0 12.9 12.9 0 0 0 4.177-2.793l86.919-86.781a12.88 12.88 0 0 0 3.776-9.109 12.88 12.88 0 0 0-3.776-9.11"
                              ></path>
                         </g>
                         <defs>
                              <clipPath id="skillIconsGit0">
                                   <path
                                        fill="#fff"
                                        d="M28 28h200v200H28z"
                                   ></path>
                              </clipPath>
                         </defs>
                    </g>
               </svg>
          ),
     },
];
export default mySkills;
